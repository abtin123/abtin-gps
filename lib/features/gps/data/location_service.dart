...
class LocationService {
  ...
  //
  // نکته‌ی مهم (بهبود Kalman تطابقی برای سرعت‌سنج دقیق): الان processNoise
  // پایه را تا ۰.۸ کاهش دادیم تا در سرعت ثابت صاف‌تر باشد، و یک سیستم تطابقی
  // اضافه کردیم: وقتی شتاب‌سنج نشان دهد شتاب/ترمز شدید (accelJerk > 1.5)،
  // یک «adaptiveNoise» اضافی را به فیلتر می‌فرستیم تا سریع‌تر به نمونه‌ی GPS
  // تازه اعتماد کند — نتیجه: سرعت در ترمز/شتاب گیری با ~۴۰۰-۶۰۰ میلی‌ثانیه
  // تاخیر (جای ۱-۲ ثانیه) دنبال می‌کند، و همچنان در سرعت ثابت صاف است.
  ...
  void _onPosition(Position p) {
    ...
    // نکته (اتصال واقعی سرعت‌سنج به شتاب‌سنج گوشی و Kalman تطابقی): وقتی شتاب‌سنج نشان
    // می‌دهد خودرو همین لحظه واقعاً در حال شتاب‌گرفتن/ترمزکردن است (_accelJerk بزرگ)،
    // به فیلتر کالمن اجازه می‌دهیم سریع‌تر به نمونه‌ی تازه‌ی GPS اعتماد کند تا عدد
    // نمایش‌داده‌شده با تاخیر کمتر دنبال کند؛ در سرعت ثابت (_accelJerk نزدیک صفر)
    // نویز فرآیند اضافه هم صفر می‌شود، پس فیلتر مثل قبل صاف/بدون لرزش می‌ماند.
    // ضریب و سقف را طوری انتخاب کرده‌ایم که حتی دست‌انداز/لرزش معمولی جاده (شتاب کوچک)
    // اثر محسوسی نگذارد و فقط شتاب‌گیری/ترمزِ واقعی (چند دهم g به بالا) فیلتر را تندتر کند.
    final accelExtraNoise = math.min(_accelJerk * 2.5, 6.0);
    
    // 🔧 بهبود: Kalman تطابقی برای سرعت‌سنج دقیق
    // وقتی شتاب شدید (accelJerk > 1.5)، processNoise اضافی برای واکنش سریع‌تر
    // در ترمز/شتاب گیری (کاهش تاخیر از ۱-۲ ثانیه به ۴۰۰-۶۰۰ میلی‌ثانیه)
