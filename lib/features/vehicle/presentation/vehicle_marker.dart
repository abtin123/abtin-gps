...
class _VehicleMarkerState extends State<VehicleMarker> {
  ...
  Future<void> _updateScreenLocation() async {
    ...
    try {
      ...
      if (!mounted) return;
      // نکته‌ی مهم (رفع اصلیِ باگ «پیکان به GPS وصل نیست، با حرکت‌دادن نقشه
      // جابه‌جا می‌شود و سر جای واقعی‌اش نمی‌ماند»):
      // نسخه‌ی قبلی اینجا هر نتیجه‌ای را که بیش از ۲۰۰ پیکسل بیرون از کادر
      // صفحه بود کاملاً نادیده می‌گرفت و آخرین مختصات معتبر را نگه می‌داشت.
      // این فرض غلط بود: وقتی کاربر نقشه را از موقعیت خودرو دور پن می‌کند،
      // خارج‌شدن مارکر از صفحه دقیقاً همان رفتار درست و مورد انتظار است (مثل
      // نقطه‌ی آبی گوگل‌مپس که وقتی نقشه را جابه‌جا می‌کنی از دید خارج
      // می‌شود). چون کد قبلی این حالت را «نامعتبر» فرض می‌کرد، مختصات
      // آخرینِ معتبر (که معمولاً نزدیک وسط صفحه بود) برای همیشه ثابت
      // می‌ماند و در نتیجه پیکان به‌جای این‌که واقعاً به مختصات GPS بچسبد،
      // عملاً یک نقطه‌ی ثابتِ نزدیک وسط صفحه بود که با هر پن جدید دوباره
      // «جابه‌جا» به‌نظر می‌رسید (چون گاهی یک به‌روزرسانی معتبر رد می‌شد و
      // گاهی نه) — دقیقاً همان چیزی که کاربر گزارش داد.
      //
      // راه‌حل درست: به toScreenLocation همیشه اعتماد کن (چون در حالت
      // followsCamera=false دوربین صاف/tilt:0 است و مختصاتش درست است — طبق
      // توضیح بالای این کلاس)، فقط مقادیر واقعاً نامعتبر (NaN/Infinity که
      // نشانه‌ی خطای داخلی پلاگین است، نه پن عادی کاربر) را فیلتر کن.
      final x = s.x.toDouble();
      final y = s.y.toDouble();
      final isValidNumber = x.isFinite && y.isFinite;
      if (isValidNumber) {
        setState(() => _screen = s);
      }
    } catch (_) {
