...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  // نکته (رفع باگ «سخنگو کار نمی‌کند»): ریشه‌ی واقعی این باگ این بود که
  // هیچ‌جای کد در طول ناوبری واقعاً TtsService.speak() را صدا نمی‌زد — فقط
  // صفحه‌ی تنظیمات صدا برای «پیش‌نمایش» صدا را صدا می‌کرد. این متغیر ایندکس
  // آخرین دستوری که با صدا خوانده شده را نگه می‌دارد تا هر دستور فقط یک‌بار
  // (نه هر تیک GPS) خوانده شود.
  ...
  Future<void> _startNavigation() async {
    ...
    // ریست ردیاب سخنگو برای سفر جدید + خواندن اولین دستور بلافاصله (به‌جای
    // این‌که کاربر تا اولین تیک GPS منتظر بماند).
  ...
  void _updateNavigationProgress(VehiclePosition pos, ActiveNavigation nav) {
    ...
    // نکته‌ی مهم (رفع اصلیِ باگ «سخنگو کار نمی‌کند»): تا امروز TtsService در
    // هیچ‌جای مسیر ناوبری صدا زده نمی‌شد؛ این تنها جایی است که در طول یک سفرِ
    // فعال دستور فعلی (instruction.text) مشخص می‌شود، پس دقیقاً همین‌جا باید
    // خوانده شود. فقط وقتی ایندکس دستور واقعاً عوض شده (نه هر تیک GPS) و
    // کاربر صدا را از منوی پایین قطع نکرده (ttEnabledProvider) می‌خوانیم، تا
    // هر دستور فقط یک‌بار گفته شود.
    if (nearestInstructionIndex != _lastSpokenInstructionIndex) {
      ...
      if (ref.read(ttEnabledProvider)) {
        final text = nav.route.instructions[nearestInstructionIndex].text;
        ref.read(ttsServiceProvider).speak(
              text,
              rate: ref.read(ttsRateProvider),
              pitch: ref.read(ttsPitchProvider),
  ...
  void _onArrived() {
    ...
    if (ref.read(ttEnabledProvider)) {
      ref.read(ttsServiceProvider).speak('به مقصد رسیدید');
    }
